...
from dqn import FF_DQN, DQNAgent
...
async def main():
    ...
    team = ConstantTeambuilder(teams["stronger_starters"])
    ...
    NUM_EPS_BASE = 100000 # number of turns (NOT GAMES)
    config = Config(
        BATCH_SIZE=32,
        GAMMA=0.99,
        EPS_START=0.95,
    ...
    policy_net = FF_DQN(n_observations, n_actions, device).to(device)
    target_net = FF_DQN(n_observations, n_actions, device).to(device)
